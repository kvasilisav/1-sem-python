...
class Bomb:
	...
	def draw(self):
		pygame.draw.rect(self.screen, self.color, (self.x - self.r, self.y - self.r, 2 * self.r, 2 * self.r))
	
	def hittest(self, obj):
		if obj.r > (self.r + self.x) and (self.y + self.r) > (obj.y - obj.r) and self.y < (obj.y + obj.r):
			return True
		else:
			return False
...
for k in range (player):
	...
	while  notfinished:
		...
		for event in pygame.event.get():
			...
			tank.move(pygame.key.get_pressed())
